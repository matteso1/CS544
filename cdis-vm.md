...
* key/password issues: your sshd may be running an old config, where it only accepts primary keys, not passwords (a problem because you need to get in with a password before you can install your public key).  In this case, sign into proxmox: https://proxmox.cs.wisc.edu.  Access your VM via the console (to get a shell, without SSH).  Check whether `sudo sshd -T | grep passwordauthentication` says "yes".  If not, try restarting SSH: `sudo systemctl restart ssh`.  Then check again (with the earlier command) to confirm password is supported (should say "yes").