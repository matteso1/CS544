...
# P7 (3% of grade): Kafka, Weather Data
...
* Please use absolute paths when reading/writing partition files in consumer.py like the following examples:
```
path = os.path.join("src", f"partition-{i}.json")
```
or
```
path = f"/src/partition-{i}.json"
```
